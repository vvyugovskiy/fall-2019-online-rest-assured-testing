...
public class ORDSTests {
    ...
    public void getOneEmployee() {
        ...
        //The path parameters. E.g. if path is "/book/{hotelId}/{roomNumber}" ->
        // -> you can do <code>get("/book/{hotelName}/{roomNumber}", "Hotels R Us", 22);</code>.
        Response response = given().baseUri(BASE_URL).when().get("/employees/{id}", 100).prettyPeek();

        // use assertions to verify respond
        int statusCode = response.statusCode(); // to save statusCode in variable
        System.out.println("statusCode :: " + statusCode);

        response.then().statusCode(200); // verify the status code
